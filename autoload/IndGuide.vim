...
"   call Decho("init s:indguide_botline_".bufnr("%")."=".s:indguide_botline_{bufnr("%")})
   while w <= wb
	...
	let prvind = (w > 1)? indent(w-1) : 0
	let curind = indent(w)
	let nxtind = (w <= line("$"))? indent(w+1) : 0
	if curind < nxtind
	 exe "sign place ".(s:INDGUIDE + 0)." line=".w." name=INDGUIDE_MORE buffer=".bufnr("%")
	elseif curind > nxtind && nxtind == 0
	 exe "sign place ".(s:INDGUIDE + 3)." line=".w." name=INDGUIDE_STOP buffer=".bufnr("%")
	elseif curind >= prvind
	 exe "sign place ".(s:INDGUIDE + 1)." line=".w." name=INDGUIDE_STAY buffer=".bufnr("%")
	elseif curind < prvind
	 exe "sign place ".(s:INDGUIDE + 2)." line=".w." name=INDGUIDE_LESS buffer=".bufnr("%")
	endif
	let w= w + 1
   endwhile
...
"	call Decho("s:indguide_botline_".bufnr("%")."=".s:indguide_botline_{bufnr("%")})
	...
	if line(".") != s:indguide_curline_{bufnr("%")} || line("w0") != s:indguide_topline_{bufnr("%")} || line("w$") != s:indguide_wbotline_{bufnr("%")} || line("$") != s:indguide_botline_{bufnr("%")}
     ...
     exe "sign unplace ".s:INDGUIDE." buffer=".bufnr("%")
     let &lz= lzkeep
    endif
   endif
...
"   call Decho("mode ".a:mode.": removal of signs")
   exe "sil! sign unplace ".(s:INDGUIDE + 0)." buffer=".bufnr("%")
   exe "sil! sign unplace ".(s:INDGUIDE + 1)." buffer=".bufnr("%")
   exe "sil! sign unplace ".(s:INDGUIDE + 2)." buffer=".bufnr("%")
   exe "sil! sign unplace ".(s:INDGUIDE + 3)." buffer=".bufnr("%")
