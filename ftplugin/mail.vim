...
" a file.
" This is now done via a plugin
" http://blog.256bit.org/exit.php?url_id=2886&entry_id=611
"fu! <SID>CheckAttach()
...
"augroup script
"    au!
"    au BufWriteCmd,FileWriteCmd mutt* :call <SID>CheckAttach()
"augroup END

"command! -nargs=1 -complete=file AttachList :call Attach("<args>")
"command! -nargs=1 -complete=file AttachFile exe "normal magg}-" | call append(line('.'), 'Attach: '.escape("<args>", " ")) | normal `a
